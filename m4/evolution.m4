...
dnl Evolution module install directory
AC_ARG_WITH(module-install-dir, [  --with-module-install-dir=PATH path to evolution module directory])
if test "x$with_module_install_dir" = "x"; then
	if test "$evolution_version_int" -ge "031191"; then
		MODULE_INSTALL_DIR=`$PKG_CONFIG --variable=moduledir evolution-shell$EVOLUTION_BASE_VERSION_S`
		if test "x$MODULE_INSTALL_DIR" = "x"; then
			MODULE_INSTALL_DIR=`$PKG_CONFIG --variable=privlibdir evolution-shell$EVOLUTION_BASE_VERSION_S`/modules
		fi
	else
		MODULE_INSTALL_DIR=`$PKG_CONFIG --variable=moduledir evolution-plugin$EVOLUTION_BASE_VERSION_S`
		if test "x$MODULE_INSTALL_DIR" = "x"; then
			AC_MSG_ERROR(Unable to find module directory)
			break;
		fi
	fi
fi
